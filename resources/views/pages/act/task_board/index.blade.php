...
@section('title', 'ACT Task Board')

@section('content')
<x-common.page-breadcrumb pageTitle="ACT Task Board" />
...
<div class="space-y-6 sm:space-y-7">
    ...
    <x-common.component-card
        title=""
        desc="Manage all Tasks Board ACT">
        ...
        <form method="GET" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 dark:bg-white/[0.03] mt-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('act.task-board.index') }}"
                ...
                <table class="w-full text-sm">
                    <thead class="border-b dark:text-white border-gray-100 dark:border-gray-800">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-gray-700 dark:text-gray-300 font-bold">Office</th>
                            <th class="px-6 py-3 text-right text-gray-700 dark:text-gray-300 font-bold">Grand Total</th>
                    ...
                    <tbody>
                            ...
                            <td class="px-6 py-4">
                                @php
                                    ...
                                    $doStatusBadge = match($doStatus) {
                                        'scm_delivered'    => ['class' => 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200', 'label' => 'Delivered'],
                                ...
                                @endphp
                                <span class="px-2 py-1 {{ $doStatusBadge['class'] }} rounded text-xs font-bold">
                                    {{ $doStatusBadge['label'] }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    @if($task->task_status !== 'completed')
                                        <a href="{{ route('act.invoices.create', ['do_id' => $task->taskable->id]) }}"
                                            class="inline-flex items-center gap-1 px-3 py-1 bg-brand-500 dark:bg-brand-600 hover:bg-brand-600 dark:hover:bg-brand-700 text-white rounded text-xs font-bold transition">
                                            Invoice
                                        </a>
                                    @endif
                                    <a href="{{ route('crm.sales-do.show', $task->taskable) }}"
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                ...
                                <p class="text-sm mt-1">Tasks will appear after deliveries are completed in SCM</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
