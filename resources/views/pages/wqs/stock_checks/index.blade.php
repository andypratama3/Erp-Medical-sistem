...
<div class="space-y-6">
    ...
    <div class="mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <a href="{{ route('wqs.task-board') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 dark:bg-blue-700 hover:bg-blue-700 dark:hover:bg-blue-800 text-white rounded-lg font-semibold transition">
            Back to Task Board
        </a>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-xl mb-6 p-4">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('wqs.stock-checks.index') }}" class="flex-1 px-4 py-2 bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600 text-gray-900 dark:text-white rounded-lg font-bold transition text-center">
                    Reset
                </a>
            </div>
        </form>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-gray-700 dark:text-gray-300 font-bold">Check Date</th>
                        <th class="px-6 py-3 text-center text-gray-700 dark:text-gray-300 font-bold">Total Items</th>
                        <th class="px-6 py-3 text-center text-gray-700 dark:text-gray-300 font-bold">Available</th>
                        <th class="px-6 py-3 text-center text-gray-700 dark:text-gray-300 font-bold">Partial</th>
                        <th class="px-6 py-3 text-center text-gray-700 dark:text-gray-300 font-bold">Not Available</th>
                        <th class="px-6 py-3 text-center text-gray-700 dark:text-gray-300 font-bold">Completion %</th>
                        <th class="px-6 py-3 text-left text-gray-700 dark:text-gray-300 font-bold">Status</th>
                        <th class="px-6 py-3 text-left text-gray-700 dark:text-gray-300 font-bold">Checked By</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($stockChecks as $check)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            <td class="px-6 py-4">
                                <a href="{{ route('crm.sales-do.show', $check->salesDO) }}" class="text-blue-600 dark:text-blue-400 hover:underline font-mono font-semibold">
                                    {{ $check->salesDO->do_code }}
                                </a>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-gray-900 dark:text-white font-medium">{{ $check->salesDO->customer?->name ?? '-' }}</p>
                                <p class="text-xs text-gray-600 dark:text-gray-400">{{ $check->salesDO->office?->name ?? '-' }}</p>
                            </td>
                            <td class="px-6 py-4 text-gray-900 dark:text-white">
                                {{ $check->check_date->format('d M Y') }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-block px-2 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 rounded font-bold">
                                    {{ $check->total_items }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-block px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded font-bold">
                                    {{ $check->available_items }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-block px-2 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 rounded font-bold">
                                    {{ $check->partial_items }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-block px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 rounded font-bold">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <div class="w-12 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                        <div class="bg-blue-600 dark:bg-blue-500 h-2 rounded-full" style="width: {{ $check->completion_percentage }}%"></div>
                                    </div>
                                    <span class="text-xs font-bold text-gray-900 dark:text-white w-8 text-right">{{ $check->completion_percentage }}%</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 bg-{{ $check->status_color }}-100 dark:bg-{{ $check->status_color }}-900 text-{{ $check->status_color }}-800 dark:text-{{ $check->status_color }}-200 rounded text-xs font-bold">
                                    {{ $check->status_label }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-gray-900 dark:text-white">
                                <p class="font-medium">{{ $check->checkedBy?->name ?? '-' }}</p>
                                <p class="text-xs text-gray-600 dark:text-gray-400">{{ $check->created_at->format('d M Y H:i') }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    <a href="{{ route('wqs.stock-checks.show', $check) }}" class="inline-flex items-center gap-1 px-2 py-1 bg-blue-600 dark:bg-blue-700 hover:bg-blue-700 dark:hover:bg-blue-800 text-white rounded text-xs font-bold transition">
                                        👁️ View
                                    </a>

                                    @if($check->overall_status !== 'completed')
                                        <a href="{{ route('wqs.stock-checks.edit', $check) }}" class="inline-flex items-center gap-1 px-2 py-1 bg-orange-600 dark:bg-orange-700 hover:bg-orange-700 dark:hover:bg-orange-800 text-white rounded text-xs font-bold transition">
                                            ✏️ Edit
                                        </a>
                                    @endif

                                    @if($check->overall_status === 'checked')
                                        <button onclick="openMarkFailedModal({{ $check->id }})" class="inline-flex items-center gap-1 px-2 py-1 bg-red-600 dark:bg-red-700 hover:bg-red-700 dark:hover:bg-red-800 text-white rounded text-xs font-bold transition">
                                            ✕ Failed
                                        </button>
                                    @endif
                                </div>
