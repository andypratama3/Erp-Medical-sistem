...
@section('content')
<div class="space-y-6">
    <x-common.page-breadcrumb pageTitle="Edit Tax" />
        ...
        <form action="{{ route('master.taxes.update', $tax) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="rate" value="{{ old('rate', $tax->rate) }}" step="0.01" required
                        class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 dark:text-white dark:border-gray-700 dark:bg-gray-900">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4"
                        class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 dark:text-white dark:border-gray-700 dark:bg-gray-900">{{ old('description', $tax->description) }}</textarea>
                </div>
            </div>

            <div class="flex items-center gap-4 mt-6">
                <button type="submit" class="inline-flex items-center justify-center font-medium gap-2 rounded-lg transition px-6 py-3 text-sm bg-brand-500 text-white shadow-theme-xs hover:bg-brand-600">
                    Update
                </button>
