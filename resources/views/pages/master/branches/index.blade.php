...
<div class="space-y-6">
    ...
    <x-common.page-breadcrumb pageTitle="Master Branches" />
    
    <form method="GET" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 dark:bg-white/[0.03]">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Search</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Code / Name / City" 
        ...
        <div class="flex items-end gap-2 mt-4">
            <a href="{{ route('master.branches.index') }}" class="inline-flex items-center justify-center font-medium gap-2 rounded-lg transition px-4 py-3 text-sm bg-red-500 text-white shadow-theme-xs hover:bg-red-600">Reset</a>
    ...
    <x-common.component-card title="Branch List" desc="Manage all branches in your system" link="{{ route('master.branches.create') }}">
        <x-table.table-component :data="$branchesData" :columns="$columns" :searchable="true" :filterable="true" :pagination="$branches" />
